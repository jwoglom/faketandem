...
func (r *Router) registerHandlers() {
	...
	// JPAKE authentication handlers (rounds 1-4). Message names/opcodes here match
	// the real protocol (pumpX2's request.authentication.Jpake1aRequest etc, opcodes
	// 32/34/36/38) -- NOTE: the real protocol also has a 5th message,
	// Jpake4KeyConfirmationRequest (opcode 40), not yet handled here.
	r.RegisterHandler(NewJPAKEHandler(r.bridge, r.jpakeManager, "Jpake1aRequest", 1))
	r.RegisterHandler(NewJPAKEHandler(r.bridge, r.jpakeManager, "Jpake1bRequest", 2))
	r.RegisterHandler(NewJPAKEHandler(r.bridge, r.jpakeManager, "Jpake2Request", 3))
	r.RegisterHandler(NewJPAKEHandler(r.bridge, r.jpakeManager, "Jpake3SessionKeyRequest", 4))
