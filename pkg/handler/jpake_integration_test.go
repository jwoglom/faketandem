			...
			if strings.Contains(line, "JPAKE_") && strings.Contains(line, "packets") {
				...
				if colonIdx > 0 {
					...
					if err := json.Unmarshal([]byte(jsonStr), &jsonData); err == nil {
						// Get the packets array and send first packet to client
						if packets, ok := jsonData["packets"].([]interface{}); ok && len(packets) > 0 {
							if hexData, ok := packets[0].(string); ok {
								t.Logf("Forwarding server response to client: %s...", hexData[:min(40, len(hexData))])
