...
- [x] Add a single-page HTML document with a header + two-column layout.
...
- [x] Connect to `ws://<host>:8080/ws` (or `wss` if the page is served over HTTPS).
- [x] On open, send `{"command":"getState"}` and update UI status pill.
- [x] On message:
...
- [x] Provide reconnect logic (exponential backoff) with a “stop reconnecting” toggle.
- **Completion criteria**:
...
- [x] Characteristic dropdown: `CurrentStatus`, `QualifyingEvents`, `HistoryLog`, `Authorization`, `Control`, `ControlStream`.
- [x] Hex input field with validation (only even-length hex).
- [x] Buttons:
...
- [x] On page load (and on demand), call `GET /api/settings`.
- [x] Render a list of message types with a compact JSON preview for each config.
- [x] Clicking a message type loads that configuration into the editor panel.
- **Completion criteria**:
...
- [x] Mode selector: `constant`, `incremental`, `time_based`.
- [x] Mode-specific editing:
...
- [x] Read-only fields (if displayed): `current_index`, `start_time`.
- **Completion criteria**:
...
- [x] “Save” button issues `PUT /api/settings/{messageType}` with a `ResponseConfig` JSON body.
- [x] “Reset State” button issues `POST /api/settings/{messageType}/reset`.
- [x] “Reload” button re-fetches the config from the server.
- **Completion criteria**:
  - Success and error messages shown in UI.
  - Save updates are reflected in the list view after reload.

### 7. CSS polish + accessibility
- [x] Basic styling for cards, headers, buttons, status pills.
- [x] Use color + text for status (don’t rely on color alone).
- [x] Ensure controls are keyboard-navigable and labels are associated with inputs.
- **Completion criteria**:
...
- [x] Add a static file handler to serve the UI at `/ui/`.
- [x] Ensure WebSocket path remains `/ws` and REST endpoints stay under `/api`.
- **Completion criteria**:
